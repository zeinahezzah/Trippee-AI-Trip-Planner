...
exports.createProgramTranslator = exports.evaluateJsonProgram = exports.createModuleTextFromProgram = void 0;
 ...
 */
function createModuleTextFromProgram(jsonObject) {
    const steps = jsonObject["@steps"];
    if (!(Array.isArray(steps) && steps.every(step => typeof step === "object" && step !== null && step.hasOwnProperty("@func")))) {
        return (0, result_1.error)("JSON object is not a valid program");
    }
    let hasError = false;
    let functionBody = "";
    let currentStep = 0;
    ...
    return hasError ?
        (0, result_1.error)("JSON program contains an invalid expression") :
        (0, result_1.success)(`import { API } from "./schema";\nfunction program(api: API) {\n${functionBody}}`);
...
exports.createModuleTextFromProgram = createModuleTextFromProgram;
/**
 ...
 */
async function evaluateJsonProgram(program, onCall) {
    const results = [];
    ...
    async function evaluate(expr) {
        return typeof expr === "object" && expr !== null ?
            await evaluateObject(expr) :
            expr;
    }
    async function evaluateObject(obj) {
        if (obj.hasOwnProperty("@ref")) {
            const index = obj["@ref"];
...
exports.evaluateJsonProgram = evaluateJsonProgram;
/**
 ...
 */
function createProgramTranslator(model, schema) {
    const translator = (0, typechat_1.createJsonTranslator)(model, schema, "Program");
...
exports.createProgramTranslator = createProgramTranslator;
